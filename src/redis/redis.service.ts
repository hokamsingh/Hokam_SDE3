...
export class RedisService implements OnModuleDestroy {
  ...
  constructor() {
    ...
    this.breaker.on('open', () =>
      this.logger.warn('Redis Circuit Breaker OPEN: Caching disabled'),
    );
    this.breaker.on('close', () =>
      this.logger.log('Redis Circuit Breaker CLOSED: Caching enabled'),
    );
  }

  async get(key: string): Promise<string | null> {
    // Wrap the call in the breaker
    return this.breaker.fire(() => this.client.get(key)) as Promise<
      string | null
    >;
  }
