...
describe('SessionsService', () => {
  ...
  describe('getSession', () => {
    it('should return cached session if available', async () => {
      ...
      (eventRepo.findBySessionId as jest.Mock).mockResolvedValue({
        events: [],
        total: 0,
      });
  ...
  describe('completeSession', () => {
    it('should update status and update cache', async () => {
      (sessionRepo.findBySessionId as jest.Mock).mockResolvedValue({
        ...mockSession,
        status: SessionStatus.ACTIVE,
      });
      (sessionRepo.updateStatus as jest.Mock).mockResolvedValue({
        ...mockSession,
    ...
    it('should throw NotFoundException if session does not exist', async () => {
      ...
      await expect(service.completeSession('999')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
